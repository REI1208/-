	...
	<div class="jy-wrapper">
		...
		<div style="height: 500px">
			...
			<div class="jy-title">
				...
				<el-button type="danger" @click="giveup">Giveup</el-button>
				<el-button type="danger" @click="regret">Regret</el-button>
			</div>
		</div>
		<div class="jy-history">
			<el-scrollbar height="500px">
				<div style="text-align: center">
					<strong>*****历史动作展示******</strong>
				</div>
...
// 获取当前棋局状态
const getBoardState = () => {
	const boardstart = game.currentLivePieceList
	// console.log("boardcast" + boardstart)
	...
	// console.log("res" + res)
	// console.log("blackPart" + blackPart)
	// console.log("redPart" + redPart)
	return res;
...
const move: MoveCallback = async (pos, cp: CheckPoint, jiangjun, penCode) => {
	...
	if (run == 'b') {
		...
		const data1: any = await request({
			url: 'http://47.107.108.89:4001/test/predictChessAction/' + cur.join(''),
			method: 'get',
		});
		// console.log("data1" + data1);
		if (data1.predictions && data1.predictions.length) {
			const predictAction = data1.predictions[0].predict_action.toString().padStart(4, '0');
			...
			game.checkDraw();
			console.log("调用后端接口查找棋谱!");
		}
		else {
			const fen = game.getCurrentPenCode('BLACK'); //获取黑方走棋的fen格式棋局
			console.log("调用中国云库接口查找棋谱!");
			console.log("fen格式棋局为：" + fen)
			const data2: any = await request({
				url:
					'http://47.107.108.89:4001/chessdb/?action=querybest&board=' + fen,
				method: 'get',
			});
			//最后四位为棋子移动status
